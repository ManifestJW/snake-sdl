...
static void debug_make_snake_long(Snake* s, int desired_len) {
    ...
    int bx = 0, by = 0;
    switch (s->dir) {
        case DIR_UP:    by =  1; break;
        case DIR_DOWN:  by = -1; break;
        case DIR_LEFT:  bx =  1; break;
        case DIR_RIGHT: bx = -1; break;
    }
    ...
    for (int i = 0; i < s->len; i++) {
        ...
        while (x < 0) x += s->grid_w;
        while (x >= s->grid_w) x -= s->grid_w;
        while (y < 0) y += s->grid_h;
        while (y >= s->grid_h) y -= s->grid_h;
...
static void Game_Reset(
    ...
    Snake_Init(snake, app->grid_w, app->grid_h, app->grid_w * app->grid_h, start_dir);
    ...
    *you_win   = false;
    ...
    // (A reset should restart gameplay, not silently change their render preference.)
...
static void SetEndTitle(SDL_Window* window, bool you_win, int score) {
    ...
    if (you_win) {
        snprintf(title, sizeof(title),
                 "snake-sdl | YOU WIN! - Continue? (L) | Score: %d",
                 score);
    } else {
        snprintf(title, sizeof(title),
                 "snake-sdl | GAME OVER - Continue? (L) | Score: %d",
                 score);
    }
...
int main(void) {
    ...
    Snake snake;
    if (!Snake_Init(&snake, app.grid_w, app.grid_h, app.grid_w * app.grid_h, start_dir)) {
        App_Shutdown(&app);
        return 1;
    }
    ...
    SnakeDrawStyle style_green = {
        .snap_head = true,
        .draw_bridges = true,
        .head_r = 0, .head_g = 255, .head_b = 0,
        .body_r = 0, .body_g = 200, .body_b = 0
    };

    SnakeDrawStyle style_blue = {
        .snap_head = true,
        .draw_bridges = true,
        .head_r = 40, .head_g = 140, .head_b = 255,
        .body_r = 40, .body_g = 120, .body_b = 220
    };
    ...
    while (running) {
        ...
        if (dt > 250000000ull) dt = 250000000ull;
        acc += dt;
        ...
        if (!game_over && !you_win) {
            if (ev.toggle_grid) show_grid = !show_grid;
            if (ev.toggle_interp) {
                // Keep both the live render flag and the "remembered" preference in sync.
                // We don't want end states (win/death) or resets to implicitly flip it.
            ...
            if (desired_hz != tick_hz) {
                ...
                // Rescale acc so the interpolation fraction stays consistent when tick_ns changes.
                if (old_tick_ns > 0 && tick_ns > 0) {
                    double frac = (double)acc / (double)old_tick_ns;
                    acc = (uint64_t)(frac * (double)tick_ns);
                }
                if (acc > tick_ns * 4) acc = tick_ns * 4;
            }
            ...
            while (acc >= tick_ns) {
                ...
                if (Apple_TryEatAndRespawn(&apple, &snake)) {
                    ...
                    if (score >= max_score) {
                        ...
                        // Freeze pose + mode on win so the final frame stays visually stable.
                ...
                if (snake_hit_self(&snake)) {
                    ...
                    float death_alpha = interp
                        ? (float)clamp01((double)acc / (double)tick_ns)
                        : 1.0f;

                    // Start death disintegration using the current interpolation fraction as the snapshot pose.
        ...
        if (game_over || you_win) {
            ...
            Fps_UpdateWindowTitle(&fps, app.window, interp, score, game_over, you_win);
        }
